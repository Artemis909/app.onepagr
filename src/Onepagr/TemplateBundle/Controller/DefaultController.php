...
class DefaultController extends Controller {
	...
	public function pageAction($page, $palette = 1) {
		...
		$entities = $mapper->addEntity($contentEntity)
		...
		$contents = $entities['content']->getData();
		$palettes = $entities['palette']->getData();
		$settings = $entities['setting']->getData();
		$contentSectionMap = $entities['contentSectionMap']->getData();

		/**
		  foreach($palettes AS $key => $value) {
		  $palettes[$key]['bgImage'] = './full/' . rand(1, 11) . '.png';
		  }* */
		// $paletteEntity->setData($palettes);
		// $mapper->addEntity($paletteEntity)->save();

		$vars = isset($palettes[$palette]) ? $palettes[$palette] : $palettes[0];
		$vars = array_merge($vars, $settings['font']);
		...
		$cssTemplate = $contentSectionMap['header']['templateCss'][0];
